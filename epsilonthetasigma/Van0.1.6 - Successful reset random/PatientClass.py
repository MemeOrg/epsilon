...
class Patient(object):
    def __init__(self,env,name,monitor,Attribute):
        ...
        self.params = {'IOPReduction':0,'time_next_visit': 0,'FirstProgression':0,
                       ...
                       'SideEffect':0,
                       'CurrentTime' : 0, 'Conversion': True,'CurrentMonth':0}
    ...
    def inCurredSideEffect(self,doctor):
        ...
        self.QALY += (0.88 - 0.101*SideEffect + 0.011*self.Attribute['MD'] - 0.065*self.medicalRecords['Cataract'])*(self.params['time_next_visit']/12)
    ...
    def params_update(self,time):
        ...
        if self.medicalRecords['CurrentMedicationType'] == 5 and self.params['IOPReduction'] > 0.05:
            self.params['IOPReduction'] -= (30/100)*(time/12)*self.params['IOPReduction']
    ...
    def onNoMedicationOrTrabeculectomy(self,time):
        ...
        if  self.medicalRecords['OnTrabeculectomy'] == True or self.medicalRecords['OnImplant'] == True:
            self.Attribute['IOP'] = self.Attribute['IOP'] *(1 + (1/100)*(time/12))
            self.medicalRecords['MedicationIntake'] += 1 
        else:
            self.Attribute['IOP'] = self.Attribute['IOP'] *(1 + (0.5/100)*(time/12))
        if self.medicalRecords['MedicationIntake'] == 0:
            self.medicalRecords['MedicationIntake'] += 1
    def onMedication(self,time):
        """Update IOP if patient is on medication"""
        self.medicalRecords['MedicationIntake'] += 1 
        self.Attribute['IOP'] = self.Attribute['IOP'] *(1-self.params['IOPReduction']*(time/12))
