...
GraphPlan = {'NoTreatment':['A'],'A':['B','E'],'B': ['C'],'C':['D','E'],'D':['E'],
...
class Doctor(object):
    ...
    def ReturnAllDoctorValues (self):
        ...
        cataractExtract = Cataract(self.PatientAttribute,self.medicalRecords)
        cataractExtract.CataractExtraction()
    def IOPTargetSetting(self):
        ...
        if self.params['VFCountdown'] > self.medicalRecords['VFinterval']: 
    ...
    def ChangeTreatmentPlan(self):
        ...
        if key == 'B' or key == 'D' or key == 'F' or key == 'NoTreatment':
            ...
            if self.PatientAttribute['Age'] < AgeNottoSurgery: 
                ...
                self.medicalRecords['TreatmentBlock'] = GraphPlan[key][1]
    def ExaminationatDoctorVisits(self):
        if self.params['Conversion'] == True:
